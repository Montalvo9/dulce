...
export default function usePedido() {
  ...
  const addItem = (item: menuItem) => {
    ...
    if (itemExiste) {
      ...
      const actualizaPedido = pedido.map((pedidoItem) =>
        pedidoItem.id === item.id
          ? { ...pedidoItem, cantidad: pedidoItem.cantidad + 1 }
          : pedidoItem,
      ); // <-- Esto es para actualizar el pedido, si el item que se está agregando al pedido ya existe en el pedido entonces se actualiza la cantidad de ese item en el pedido, esto se hace usando el método map para recorrer el pedido y encontrar el item que se está agregando al pedido, si se encuentra el item entonces se actualiza la cantidad de ese item en el pedido sumando 1 a la cantidad actual, si no se encuentra el item entonces se deja igual
